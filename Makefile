NAME		= libft.a
CC			= gcc
FLAGS		= -Wall -Wextra -Werror -I./includes
SRCS		= ft_bzero.c ft_memset.c ft_memcpy.c \
			  ft_strlen.c ft_memccpy.c ft_memmove.c \
			  ft_memchr.c ft_memcmp.c ft_strlcpy.c \
			  ft_strlcat.c ft_strchr.c ft_strrchr.c \
			  ft_strnstr.c ft_strncmp.c ft_atoi.c \
			  ft_isalpha.c ft_isdigit.c ft_isalnum.c \
			  ft_isascii.c ft_isprint.c ft_toupper.c \
			  ft_tolower.c ft_calloc.c ft_strdup.c \
			  ft_substr.c ft_strjoin.c ft_strtrim.c \
			  ft_split.c ft_itoa.c ft_strmapi.c \
			  ft_putchar_fd.c ft_putstr_fd.c ft_putendl_fd.c \
			  ft_putnbr_fd.c ft_lstnew.c ft_lstadd_front.c ft_lstsize.c \
			  ft_lstlast.c ft_lstadd_back.c ft_lstdelone.c \
			  ft_lstclear.c ft_lstiter.c ft_lstmap.c \
			  ft_flag_proc.c ft_parse_flag.c ft_printf.c \
			  ft_proc_char.c ft_proc_hex.c ft_proc_int.c \
			  ft_proc_percent.c ft_proc_pointer.c ft_proc_string.c \
			  ft_proc_uint.c ft_processor.c \
			  get_next_line.c \

OBJ_NAME	= $(SRCS:.c=.o)
OBJ			= $(addprefix $(OBJ_DIR),$(OBJ_NAME))
SRC_DIR 	= srcs/
OBJ_DIR 	= objs/
LIBC		= ar rcs
RM			= rm -f

all:		${NAME}

${NAME}:	${OBJ}
			${LIBC} ${NAME} ${OBJ}
			@echo "##### libft created #####"

$(OBJ_DIR)%.o:	$(SRC_DIR)%.c
				@mkdir -p $(OBJ_DIR)
				${CC} $(FLAGS) -o $@ -c $< $(INC)
clean:
			${RM} ${OBJ}
			@echo "##### Removed libft object files #####"

fclean:		clean
			${RM} ${NAME}
			@echo "##### Removed libft.a #####"

re:			fclean all


.PHONY:		all clean fclean re